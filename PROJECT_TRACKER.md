...
- **State of the Code:** 
  ...
  - Stage 2 completed: Travel/intercept math consolidated into `v1_1/physics.py`, eliminating duplicates in `targeting.py`. Precomputed distance table added for rigid-body inner planet co-rotation and stationary outer planets.
  - `v1_1` parity verified vs `v1` with a perfect 50% win rate across 10 games in the local arena.

---

## §8 — Open Questions & Blockers 🟢 *(overwrite each session)*

- **Blockers:** Stage 0 (Q2 Blocker: same-owner combat-combination rule in vendored engine) is currently being addressed by another LLM worker.
...
- `D-007 | 2026-06-17 | V2 Information Model | Moved from per-turn dense snapshot to event-based timeline forecast. Implemented rigid-body inner planet rotation distance caching. | Replaces naive sum-based defense calculation. Frees up 'frozen capital'. | §3, §7`
...
- `S-007 | 2026-06-17 | v1.1 | Completed Stage 2: Consolidated travel_time and lane_clear into physics.py. Added stationary distance table. Verified parity vs v1.`
