...
- **Last updated:** 2026-06-18 — Session 8 (garrison-release verify-first: NOT the binding constraint — lever A reverted as a no-op; launch gate identified; lever B parked)
- **File version:** v1.0
- **Owner:** Gary Mei
...
- **Phase:** v2 Development — macro tuning in **`v2_1/`** (active dev dir). Travel-time valuation landed (D-016, beats `v2_macro` 57%). **Session 8 ran the garrison-release plan's verify-first step and ruled garrison release OUT as the binding constraint** (D-020): the hoard is the launch/target gate, not `min_garrison`. Lever A reverted — code is back at the 57% baseline. New direction: the launch gate (see §9).
- **State of the Code:** 
  ...
  - **Session 8 — Garrison release verified NOT binding; lever A reverted (D-020).** Instrumented `decide()` with a `GARRISON_DEBUG`-gated probe (owned / released / sent / leftover / pos_targets / will_lose) and ran one `producer_lite` game in-process (`scripts/verify_garrison.py`, seed 12345). Findings: (1) **early game (1 base, steps 0–14) capital is already released** — `released ≈ owned`, `wl=0` — yet `sent=0` most turns; the agent only launches when it can fully fund a capture, so a lone base drip-feeds ~29-ship lumps every ~6 turns and expansion stalls at 3 planets by step 15. (2) **Lever A (shrink the freeze lookahead 30→12) is a confirmed no-op:** `wl=0` throughout and `released` did not move a single ship — the freeze isn't far threats or `will_lose`. (3) The **mid-game freeze is the M2 trough** (step 40: `released=32 / owned=205`, `wl=0`) reacting to a <12-turn incoming hit; it reserves ~173 ships and loses the planet anyway by step 50–60 — real but secondary. (4) `pos_targets` stays 8–16 until the step-50 collapse, so **M3 is not the early wall** either. Conclusion: the binding constraint is the all-or-nothing **launch/target gate** (`strategy.decide` step 4) that won't spend capital that is *already free*. Reverted lever A to the validated 57% baseline; kept `scripts/verify_garrison.py` as the harness. **Lever B (reserve only the first-threat survival force, release the surplus) is parked** as the future fix for the M2 trough.
...
- **Blockers:** None.
- **Q1 (Strategy) — RE-DIAGNOSED (Session 8, D-020).** Garrison release is **not** the binding constraint. Verify-first showed early-game capital is *already released* (`released ≈ owned`, `will_lose=0`) but unspent: `strategy.decide` step 4 only launches a **fully funded capture**, so a lone base drip-feeds ~29-ship lumps and expansion stalls at 3 planets (~step 15). The binding lever is the **launch/target gate**, not `min_garrison`.
- **Q2 (Strategy) — open.** Should "snipe" be a first-class, reachability-gated valuation term in `economy.py`/`strategy.py`? Still absent; deferred behind the launch-gate fix.
- **Q5 (Strategy) — open (new).** How to make the launch gate spend already-free capital: partial commits toward the cheapest reachable neutral, parallel cheap-neutral expansion, or a per-turn deploy budget? Needs design + arena. This is now Next Action #1.
- **Q3 (Engine) — RESOLVED.** Same-owner co-arrivals stack (fleets combine). Confirmed Stage 0 (D-007).
- **Q4 (Hygiene) — RESOLVED (Session 7, D-018).** Doc drift reconciled `v1_1`→`v2_1`; only intentional `v1_1` strings remain (lineage note + real `tests/test_v1_1_*.py` filenames).

---

## §9 — Next Actions 🟢 *(overwrite each session)

1. **Fix the launch/target gate (new binding constraint, per D-020).** Let a base deploy capital that is *already free* instead of waiting to fully fund one capture — e.g. partial commits toward the cheapest reachable neutral, parallel cheap-neutral expansion, or a per-turn deploy budget. First instrument per-candidate `need` vs `available` (why `accumulated < need`) on a `Producer Lite` game (`scripts/verify_garrison.py` is the harness), then arena-gate vs `v2_macro` (≥ parity, today 57%) + `producer_lite` (watch margin ~−4200 shrink + planet count rise past 3).
2. *(Parked)* **Lever B — first-threat garrison reserve.** Replace `will_lose→available=0` / the 30-turn trough with "reserve only enough to survive the first committed enemy arrival (`f_enemy` by `t_enemy`, via `physics.arrival_garrison` / `required_to_capture`), release the surplus." Targets the mid-game M2 freeze (step 40: 173 of 205 frozen). Real but secondary to #1.
3. **Add a reachability-gated snipe term** (`economy.py`/`strategy.py`) — value capturing a planet at its post-combat garrison trough only when the reachability race confirms a fleet lands inside the trough window. Deferred.
...
- `D-019 | 2026-06-18 | Add a "canary in the coal mine": address the user as Gary and open every message with "Gary -", instructed in AGENTS.md, CLAUDE.md, and GEMINI.md. | A visible tripwire proving the contract/project-memory actually loaded into context. A reply not opening with "Gary -" is a dead canary — the loader (CLAUDE/GEMINI) or contract (AGENTS) was never read, so its instructions aren't in effect. Placed in all three entry points so it fires whichever loader is active. | All (contract)`
- `D-020 | 2026-06-18 | Garrison release is NOT the binding constraint — revert lever A, pivot to the launch/target gate, park lever B. | Verify-first (a GARRISON_DEBUG probe over one producer_lite game) showed early-game capital is already released (released≈owned, wl=0) but unspent: decide() step 4 only launches a fully funded capture, so a lone base drip-feeds ~29-ship lumps and stalls at 3 planets. Lever A (freeze lookahead 30→12) moved released by 0 ships (wl=0 throughout) — a confirmed no-op. The mid-game freeze is the M2 trough (step 40: 32 released / 205 owned) reacting to a <12-turn hit, losing the planet anyway (secondary). Binding constraint = the all-or-nothing launch/target gate, not min_garrison. Reverted to the 57% baseline; kept scripts/verify_garrison.py; parked lever B (first-threat reserve) for the M2 trough. | §7, §8, §9`
...
- `S-016 | 2026-06-18 | v2_1 | Garrison-release verify-first. Instrumented decide() (GARRISON_DEBUG probe: owned/released/sent/leftover/pos_targets/will_lose) + added scripts/verify_garrison.py (one in-process producer_lite game). Found garrison release is NOT binding: capital already released early (released≈owned, wl=0) but the all-or-nothing launch gate won't spend it (single-base ~29-ship lumps, stalls at 3 planets); lever A (freeze lookahead 30→12) a verified no-op; mid-game M2 trough freeze real but secondary (loses anyway). Reverted lever A to the 57% baseline; kept the harness. Logged D-020; pivoted Next Action #1 to the launch/target gate; parked lever B.`
