...
def _resolve_spec(spec):
    ...
    import importlib
    if spec in ("main.py", "v1/agent.py", "agent.py"):
        return getattr(importlib.import_module("v1.agent"), "act")
    if spec in ("v2_macro/agent.py", "v1_1/agent.py"):  # v1_1 was renamed to v2_macro
        return getattr(importlib.import_module("v2_macro.agent"), "act")
    if spec == "v2_1/agent.py":
        return getattr(importlib.import_module("v2_1.agent"), "act")
    if spec in ("producer", "producer_lite", "opponents/producer_lite/opp_producer.py"):
        import importlib.util, os
        path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                            "..", "opponents", "producer_lite", "opp_producer.py")
        s = importlib.util.spec_from_file_location("opp_producer", path)
        m = importlib.util.module_from_spec(s)
        sys.modules[s.name] = m  # register so @dataclass/torch can resolve __module__
