...
def main():
    ...
    import math
    px, py = ph.planet_position(50.0, 60.0, 1.0, math.pi/2, 1)  # 1/4 turn around center (50, 50)
    assert math.isclose(px, 40.0, abs_tol=1e-9)
    assert math.isclose(py, 50.0, abs_tol=1e-9)
    assert ph.intercept_time(0.0, 0.0, lambda t: (100.0, 0.0) if t < 100 else None, 5.0) == 20
